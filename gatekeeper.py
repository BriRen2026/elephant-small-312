...
#Flask-gatekeeper written by k0rventen
#https://github.com/k0rventen/flask-gatekeeper/blob/main/flask_gatekeeper/gatekeeper.py
...
class GateKeeper:
    def __init__(self, app: Flask = None, ban_rule: dict = None, rate_limit_rules: list = None, ip_header: str = None,
    ...
    def _ban_func(self, ban_infos):
        ...
        Returns:
            Response: ready to be server response with 429 http code and retry after header
        """
    ...
    def _rate_limit_func(self, rate_limit_infos):
        ...
        """
        #IMMEDIATELY Report a user if they have breached the rate_limit
        self.report()

        #rate_limit_response = Response("ip {} rate limited for {}s (over {} requests in a {}s window)".format(
        #    rate_limit_infos["ip"], rate_limit_infos["retry"], rate_limit_infos["count"], rate_limit_infos["window"]),
        #    status=429)
    ...
    def _before_request(self):
        ...
        if request.method not in self.excluded_methods and request.endpoint not in self.bypass_routes:
            ...
            # Force add 2 entries for HTML and Favicon ONLY for 1 request. ALL pages will have this, so not an issue
            if "css" in request.endpoint:
    ...
    def _is_ip_banned(self, ip) -> bool:
        ...
        if not self.ips[ip].ban_active_until:
            if len([e for e in self.ips[ip].ban_entries if e >= time_now - self.ban_rule["window"]]) >= self.ban_rule[
                "count"]:
                banned_for = int((self.ips[ip].ban_entries[-1] + self.ban_rule["duration"]) - time_now)
                self.ips[ip].ban_active_until = time_now + banned_for
                return {"ip": ip, "window": self.ban_rule["window"], "count": self.ban_rule["count"],
                        "retry": int(banned_for)}
            return None
